...
| 版本 | 檔案 | 日期 | 說明 |
|---|---|---|---|
| v1.0.4 | index.html | 2026-08-19 | 磅數修正、結單覆蓋 bug fix（見下方詳細） |
| v1.0.3 | index.html | 2026-07-04 | 同步功能修正（見下方詳細） |
| v1.0.2 | index.html / index_ios.html | 2026-06-26 | 手機操作體驗（已併入 index.html，見下方詳細） |
| v1.0.1 | index.html | 2026-06-26 | UI 更新（見下方詳細） |
| v1.0.0 | index.html | 2026-06-24 | First Release（見下方詳細） |

### v1.0.4 — 2026-08-19 · 磅數修正、結單覆蓋 bug fix

> 本版以**線上部署版（v1.0.3）**為基礎修改。本地 `index.html` 當時停在 v1.0.1、
> 落後線上兩個版本，直接沿用會回退 v1.0.2 / v1.0.3 的成果，
> 因此改以 `curl` 抓回線上檔案再套用修正。舊的本地檔備份為 `index_stale_backup.html`。

**磅數修正**

- 原 `openDetail()` 將商品詳情價格硬編為 `NT$xxx / 0.25磅`，所有品項都顯示 0.25磅
- 新增 `productSize(p)`：依 `p.size` → 品名 / `url` / `id` 是否含 `4oz` 或 `0.25磅`
  判斷，其餘回傳 `0.5磅`（對應官網小叮嚀：高單價及數量少的豆子才用 0.25磅袋裝）
- **不採用在 `DEFAULT_PRODUCTS` 逐筆加 `size` 的做法**：正式環境 `products` 集合
  早已被官網同步結果整批取代（文件 ID 為 Kakalove handle），`DEFAULT_PRODUCTS`
  僅在集合為空時植入，改它對線上品項毫無作用
- 以線上 44 筆實際品項驗證：10 筆判為 `0.25磅`、34 筆判為 `0.5磅`，與品名標示相符

**結單覆蓋 bug fix**

- `finalizeRound()` 原以 `history/{round}` 為文件 ID 呼叫 `.set()`，
  同輪次重複結單會**靜默覆蓋**既有紀錄（實際造成第一輪團購紀錄遺失且無法復原）
- 改為 `.add()` 自動生成 ID；寫入前先 `where('round','==',roundNum).limit(1)`
  檢查重複，已存在則中止並提示
- 刪除 / 收據更新既有邏輯已使用 `rec._docId`（來自 `onSnapshot` 的 `d.id`），與自動 ID 相容

### v1.0.3 — 2026-07-04 · 同步功能修正

- 修正同步功能受促銷頁面影響導致風味 / 烘焙度抓取錯誤
- 修正「編輯後新增」烘焙度欄位未正規化導致儲存空值
- 同步清單新增簡介顯示
...
- 磅數由 `productSize()` 從品名 / URL 推斷，非官網欄位。若 Kakalove 改用其他
  袋裝規格（如 1 磅）或品名未標示 `0.25磅` / `4oz`，會誤判為 `0.5磅`
- 正式環境的 `products` 集合已被官網同步結果整批取代（文件 ID 為 Kakalove handle，
  非 `DEFAULT_PRODUCTS` 的短 id），修改 `DEFAULT_PRODUCTS` 不會影響線上品項
- **本地與線上版本容易失焦**：過去曾發生本地 `index.html` 落後線上兩版的情況。
  修改前建議先 `curl https://cold-disk-bf47.tzuweichengkor.workers.dev/` 抓回線上版比對
